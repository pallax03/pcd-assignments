...
public class Ball {
    ...
    private static boolean isBallsNear(Ball first, Ball second) {
        double dx = second.getPos().x() - first.getPos().x();
        double dy = second.getPos().y() - first.getPos().y();
        double distSq = dx * dx + dy * dy;
        double minR = first.getRadius() + second.getRadius();

        return distSq <= minR * minR;
    }

    public static boolean checkAndResolveCollisionSafe(Ball first, Ball second) {
        if (!isBallsNear(first, second)) {
            return false;
        }

        Ball firstLock = second;
        Ball secondLock = first;
        ...
        synchronized (firstLock) {
            synchronized (secondLock) {
                if (!isBallsNear(firstLock, secondLock)) {
                    return false;
                }
                return resolveCollision(firstLock, secondLock);
            }
        }
    }
    ...
    public void setLastHitBy(Who actor) {
        this.lastHitBy = actor;
    }

    public Who getLastHitBy() {
